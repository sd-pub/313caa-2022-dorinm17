 ...
 * este corect parantezat, intorcand 1, si 0, in caz contrar.
 */
int
is_valid(char *brackets) 
{
    stack_t *stack = st_create(MAX_STRING_SIZE);

    while (*brackets) {
        if (is_open(*brackets)) {
            st_push(stack, &(*brackets));
        } else if (st_is_empty(stack)) {
            st_free(stack);
            return 0;
        } else if (is_match(*((char *)st_peek(stack)), *brackets)) {
            st_pop(stack);
        } else {
            st_free(stack);
            return 0;
