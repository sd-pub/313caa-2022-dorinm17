...
b_tree_create(size_t data_size)
{
    ...
    DIE(!b_tree, "b_tree malloc failed\n");
 ...
 */
b_node_t *find_lca(b_node_t *b_node, int n1, int n2)
{
    if (!b_node)
        return NULL;

    if (*((int *)b_node->data) == n1 || *((int *)b_node->data) == n2)
        return b_node;
 
    b_node_t *left_lca = find_lca(b_node->left, n1, n2);
    b_node_t *right_lca = find_lca(b_node->right, n1, n2);
...
int b_tree_lca(b_tree_t *b_tree, int n1, int n2)
{
    ...
    b_node_t *lca = find_lca(b_tree->root, n1, n2);
    return *((int *)lca->data);
}
