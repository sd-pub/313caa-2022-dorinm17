...
void
dfs_list_graph(list_graph_t *lg, int node, int *color, linked_list_t *cc)
{
    ...
    while (!st_is_empty(st)) {
        int curr_node = *((int *)st_peek(st));
        int neighbor_exist = 0;
		ll_node_t *curr = lg->neighbors[curr_node]->head;
        ...
        if (!neighbor_exist) {
            color[curr_node] = 2;
			ll_add_nth_node(cc, cc->size, &curr_node);
            st_pop(st);
        }
    }
...
void print_connected_components(list_graph_t *graph)
{
	...
	DIE(!color, "color calloc failed\n");
	linked_list_t *cc = ll_create(sizeof(int));

	int new = -1, ct = 0;
	for (int i = 0; i < graph->nodes; i++)
		if (color[i] == 0) {
			dfs_list_graph(graph, i, color, cc);
			ll_add_nth_node(cc, cc->size, &new);
			ct++;
		}
	
	printf("%d\n", ct);
	ll_node_t *curr = cc->head;
	while (curr) {
		if (*((int *)curr->data) == -1)
			printf("\n");
		else
